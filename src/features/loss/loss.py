...
from features.boxes.boxes import box_cxcywh_to_xyxy, generalized_box_iou


class HungarianMatcher(nn.Module):
    """
    Computes an assignment between ground truth and predictions using the Hungarian algorithm.
    Uses classification, bbox, and GIoU costs for matching.
    """

    def __init__(self, weight_dict: Dict[str, float]) -> None:
        """
        Args:
            weight_dict (Dict[str, float]): Contains weights for 'class_weighting', 'bbox_weighting', 'giou_weighting'.
        """
        super().__init__()
        assert (
            weight_dict.get("class_weighting") is not None
            and weight_dict.get("bbox_weighting") is not None
            and weight_dict.get("giou_weighting") is not None
        ), "Weight dict must contain weighting for all three losses, giou, class and bbox."
        assert (
            weight_dict.get("class_weighting") != 0
            or weight_dict.get("bbox_weighting") != 0
            or weight_dict.get("giou_weighting") != 0
    ...
    @torch.no_grad()
    def forward(
        self, yhat: Dict[str, torch.Tensor], y: List[Dict[str, torch.Tensor]]
    ) -> List[Tuple[torch.Tensor, torch.Tensor]]:
        """
        Performs the matching for each batch element.

        Args:
            yhat (Dict[str, torch.Tensor]): Contains 'pred_logits' and 'pred_boxes'.
            y (List[Dict[str, torch.Tensor]]): List of targets, each with 'labels' and 'boxes'.

        Returns:
            List[Tuple[torch.Tensor, torch.Tensor]]: Tuples of (prediction indices, target indices) per batch.
        """
        indices = []
        for batch_idx, target in enumerate(y):
            ...
            cost_giou = -generalized_box_iou(
                box_cxcywh_to_xyxy(batch_boxes), box_cxcywh_to_xyxy(tgt_boxes)
            )
...
class DETRLoss(nn.Module):
    """
    Computes the loss for DETR. Uses Hungarian matching and supervises matched pairs for class and box losses.
    """
    ...
    def __init__(
        ...
        """
        Args:
            num_classes (int): Number of object categories (excluding no-object).
            matcher (HungarianMatcher): Matching module.
            weight_dict (Dict[str, float]): Loss weights.
            eos_coef (float): Classification weight for the no-object category.
        """
        super().__init__()
    ...
    def classification_loss(
        self,
        yhat: Dict[str, torch.Tensor],
        y: List[Dict[str, torch.Tensor]],
        indices: List[Tuple[torch.Tensor, torch.Tensor]],
    ) -> Dict[str, torch.Tensor]:
        """
        Compute classification loss for matched pairs.
        ...
        Returns:
            Dict[str, torch.Tensor]: Classification loss.
        """
    ...
    def box_loss(
        ...
        """
        Compute bounding box regression and GIoU losses for matched pairs.
        ...
        Returns:
            Dict[str, torch.Tensor]: Box and GIoU losses.
        """
    ...
    def get_matched_query_indices(
        self, indices: List[Tuple[torch.Tensor, torch.Tensor]]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns batch and query indices for matched pairs.

        Args:
            indices: List of (query_indices, target_indices) per batch.

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Batch indices and query indices.
        """
    ...
    def forward(
        ...
        Returns:
            Dict[str, Dict[str, torch.Tensor]]: Losses for 'labels' and 'boxes'.
        """
        indices = self.matcher(yhat, y)
        device = next(iter(yhat.values())).device
        y = [
            {"labels": t["labels"].to(torch.long), "boxes": t["boxes"].to(torch.float32)} for t in y
        ]
    ...
    y = [
        {
            ...
            "boxes": torch.tensor(
                [[0.5, 0.5, 0.2, 0.3], [0.3, 0.7, 0.1, 0.2], [0.8, 0.2, 0.15, 0.25]]
            ),
        },
